...
void save_file_attrs(const std::vector<FileAttr>& attrs, const std::string& savFile) {
    std::ofstream out{ savFile };
    char buf[32];
    
    for (const FileAttr& attr : attrs) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cerr << "meet system error, " << se.code().value() << ", " << se.what() << "\n";
        return -1;
    }
