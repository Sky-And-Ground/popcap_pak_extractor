...
void save_single_file_body(HANDLE handle, FileAttr* attr, i32 write_buffer_size, const char* path) {
    ...
    while (file_size > 0) {
        ...
        if (!WriteFile(handle, buffer, read_len, NULL, NULL)) {
            log_error_exit("cannot write body data to %s, %s\n", attr->name, win_strerr(GetLastError()));
        }
    ...
    if (!SetFileTime(handle, NULL, NULL, &(attr->last_write_time))) {
        log_error_exit("cannot set file time to %s, %s\n", attr->name, win_strerr(GetLastError()));
    }
}
...
void save_pak_body(const char* to_dir) {
    ...
    while (cursor) {
        ...
        write_handle = CreateFile(path, GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);

        if (write_handle == INVALID_HANDLE_VALUE) {
            log_error_exit("cannot create file handle to save body of %s, %s\n", cursor->name, win_strerr(GetLastError()));
        }

        save_single_file_body(write_handle, cursor, write_buffer_size, to_dir);
