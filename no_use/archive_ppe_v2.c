...
static UCHAR magic[BYTES_OF_MAGIC];
static UCHAR version[BYTES_OF_VERSION];
static struct FileAttrList* attrList = NULL;
static HANDLE pakHandle = INVALID_HANDLE_VALUE;
static DWORD pakFileSize = 0;
static HANDLE tempFileHandle = INVALID_HANDLE_VALUE;
static char writeBuf[WRITE_BUF_SIZE];
...
void init_all(const char* pakPath);
...
void init_all(const char* pakPath) {
    ...
    pakHandle = CreateFile(pakPath, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (pakHandle == INVALID_HANDLE_VALUE) {
        err = GetLastError();
        fprintf(stderr, "%s: sys call CreateFile failed on \"%s\", %ld, %s\n", __func__, pakPath, err, win_strerr(err));
        exit(EXIT_FAILURE);
    }

    pakFileSize = GetFileSize(pakHandle, NULL);
    if (pakFileSize == INVALID_FILE_SIZE) {
        err = GetLastError();
        fprintf(stderr, "%s: sys call GetFileSize failed on \"%s\", %ld, %s\n", __func__, pakPath, err, win_strerr(err));
        exit(EXIT_FAILURE);
    }

    attrList = create_file_attr_list();
}

void destroy_all(void) {
    if (attrList) {
        destroy_file_attr_list(attrList);
    }

    if (pakHandle != INVALID_HANDLE_VALUE) {
        CloseHandle(pakHandle);
    }
            ...
            *cursor = '\0';
            ...
            if (!is_dir_exist(path)) {
                if (!CreateDirectory(path, NULL)) {
                    err = GetLastError();
                    fprintf(stderr, "%s: sys call CreateDirectory failed on \"%s\", %ld, %s\n", __func__, path, err, win_strerr(err));
                    exit(EXIT_FAILURE);
                }
            }
...
struct FileAttr* create_file_attr(void) {
    struct FileAttr* attr = (struct FileAttr*)malloc_or_exit(sizeof(struct FileAttr));
    attr->name = NULL;
    attr->next = NULL;
    return attr;
}
...
struct FileAttrList* create_file_attr_list(void) {
    struct FileAttrList* list = (struct FileAttrList*)malloc_or_exit(sizeof(struct FileAttrList));
    
    list->head = (struct FileAttr*)malloc_or_exit(sizeof(struct FileAttr));
    list->head->next = NULL;
    list->tail = list->head;
    list->length = 0;
    return list;
...
DWORD pak_read(char* buf, DWORD numOfBytesToRead) {
    ...
    if (pakFileSize < numOfBytesToRead) {
        fprintf(stderr, "%s: remained file size is less than required, file maybe broken\n", __func__);
        exit(EXIT_FAILURE);
    }

    if (!ReadFile(pakHandle, (LPVOID)buf, numOfBytesToRead, &readLen, NULL)) {
        err = GetLastError();
        fprintf(stderr, "%s: sys call ReadFile failed, %ld, %s\n", __func__, err, win_strerr(err));
        exit(EXIT_FAILURE);
    }
...
void pak_parse_magic(void) {
    pak_read((char*)magic, sizeof(magic));
    decode_bytes(magic, magic, BYTES_OF_MAGIC);
}

void pak_parse_version(void) {
    pak_read((char*)version, sizeof(version));
...
void pak_parse_file_name(struct FileAttr* attr) {
    ...
    attr->name = (char*)malloc_or_exit((fileNameLen + 1) * sizeof(char));
...
void pak_parse_all_file_attrs(void) {
    ...
    while (1) {
        ...
        file_attr_list_add(attrList, attr);
    }
}
...
void pak_parse_header(void) {
    ...
    if (!is_magic_valid()) {
        fprintf(stderr, "%s: magic is not valid\n", __func__);
        exit(EXIT_FAILURE);
    }
    ...
    if (!is_version_valid()) {
        fprintf(stderr, "%s: version is not valid\n", __func__);
        exit(EXIT_FAILURE);
    }
...
void pak_parse_single_file_body(struct FileAttr* attr, const char* extractDir) {
    ...
    if (!SetFileTime(tempFileHandle, NULL, NULL, &(attr->lastWriteTime))) {
        err = GetLastError();
        fprintf(stderr, "%s: sys call SetFileTime failed, %ld, %s\n", __func__, err, win_strerr(err));
        exit(EXIT_FAILURE);
    }
...
void pak_parse_body(const char* extractDir) {
    struct FileAttr* cursor = attrList->head->next;
